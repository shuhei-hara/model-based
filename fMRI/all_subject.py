...
# cmap = cmap_timing
    second_level_model = SecondLevelModel(smoothing_fwhm=8) #4~8 if you want to detect fine-grained 4, if you want broader regions 8.
    ...
    plotting.plot_stat_map(
        z_map, threshold=3.0,
        title=f'{contrast}')
    plt.savefig(os.path.join(group_dir,f'{contrast}_raw_z.jpg'))

    z_image_path = os.path.join(group_dir, f'{contrast}_z_map.nii.gz')
    ...
    plotting.plot_stat_map(
        thresholded_map,
        threshold=threshold,
        title="Thresholded z map, fpr<.0001, clusters > 10 voxels",
    )
    plt.savefig(os.path.join(group_dir,f'{contrast}(fdr,cluster).jpg'))

    report = make_glm_report(model=second_level_model,contrasts=['group'], threshold=threshold, cluster_threshold=number_cluster, height_control=height_control,alpha=alpha,two_sided=True)
    report.save_as_html(os.path.join(group_dir,f'report_{contrast}.html'))
